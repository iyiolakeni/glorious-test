...
{ /* char is stored at left and paddind at buffer's right */
	...
	if (width > 1)
	{
		...
		for (i = 0; i < width - 1; i++)
			buffer_array[BUFF_SIZE - i - 2] = padd;

		if (flags & _MINUS)
			return (write(1, &buffer_array[0], 1) +
					write(1, &buffer_array[BUFF_SIZE - i - 1], width - 1));
		else
			return (write(1, &buffer_array[BUFF_SIZE - i - 1], width - 1) +
 ...
 * Return: Number of chars printed.
 */
int _number_printer(int is_negative, int ind, char buffer_array[],
	int flags, int width, int precision, int size)
{
	...
	return (_printer_number(ind, buffer_array, flags, width, precision,
		length, padd, extra_ch));
}

/**
 * _printer_number - Write a number using a bufffer
 ...
 */
int _printer_number(int ind, char buffer[],
